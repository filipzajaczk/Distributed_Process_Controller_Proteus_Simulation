...
void LcdInit(void)
 ...
 void Lcd_DefineChar(unsigned char code_pos, unsigned char *pattern)
{
	unsigned char i;
	Lcd_WriteControl(0x40 + (code_pos << 3)); 
	...
	Lcd_WriteControl(0x80); 
}

void Lcd_Clear(void)
{
    Lcd_WriteControl(0x01); // 0x01 to komenda Clear Display
    // Musimy poczekac, bo czyszczenie trwa dluzej (ok 1.5ms - 2ms)
    // Uzywamy istniejacej funkcji opoznienia w LCD.c (np. w5mS lub wlasnej petli)
    // Zakladam ze w5mS() jest dostepne w Twoim projekcie:
    w5mS(); 
}

// Funkcja wypisujaca tekst (string)
